...
import { OperationFilterType } from "@/lib/workflow_editor/types/w_types";
import {
...
type ExtendedOperationFilterType = OperationFilterType<
  (vsAnyPrimitives | vsAnyRawTypes)["type"]
>;

type Props = {
  onDelete: () => void;
  onSave: (filterObj: ExtendedOperationFilterType) => void;
  initialFilter: ExtendedOperationFilterType;
  index: number;
};

const FilterInput = ({ onDelete, onSave, initialFilter, index }: Props) => {
  const filterContainerRef = useRef<HTMLDivElement>(null);
  const [isFilterContainerSelected, setIsFilterContainerSelected] =
    useState<boolean>(true);
  const clonedInitialFilter = structuredClone(initialFilter);

  const [filterObj, setFilterObj] = useState<ExtendedOperationFilterType>({
    keyId: clonedInitialFilter.keyId,
    filterCriteria: clonedInitialFilter.filterCriteria as any,
    filterValue: [clonedInitialFilter.filterValue as any],
    filterType: clonedInitialFilter.filterType as any,
    inputID: clonedInitialFilter.inputID ?? "",
  });
  ...
  const saveFilter = (): boolean => {
    ...
    valueInputsSchema.forEach((inputSchemaAt, id) => {
      ...
      if (
        inputSchemaAt === "date" &&
        (inputValue === null || !isValidISODateString(inputValue?.toString()))
      ) {
      ...
      if (
        ...
        (inputValue === null || isNaN(Number(inputValue)))
      ) {
  ...
  const getInputType = (
    ...
    const inputPaths: string[] = toStringSafe(inputID).split(".");
    return getType(fakeInputs, inputPaths, 0);
  };
    ...
    >
      {!isFilterContainerSelected ? (
        ...
        <>
            ...
            <div className="flex flex-1 min-w-[30%]">
              <MultiSelect
                ...
                handleSelect={(selectedCriteria) => {
                  ...
                  if (selectedCriteriaInputSchema !== valueInputsSchema) {
                    setFilterObj((prev) => ({
                      ...prev,
                      filterValue: [undefined] as any,
                    }));
                ...
                {valueInputsSchema.map((inputSchema, idx) => {
                  return (
                    <FilterValueInput
                      ...
                      onSave={(inputValue) => {
                        let arr = structuredClone(filterObj.filterValue);
                      ...
                      key={`${
                        filterObj.keyId
                      }_filtervalue_${generateHexRandomString(10)}`}
          ...
          <div className="w-full flex flex-1 gap-1 items-center justify-end">
            {/* Delete: Button */}
            <Button
              variant={"ghost"}
              className={cn(
...
export const TextWithSeparator = ({
  ...
  const getContentInline = (content: any): string => {
    ...
    if (!isNaN(Number(content)) && isDate(content)) {
      return _formatDate(content);
