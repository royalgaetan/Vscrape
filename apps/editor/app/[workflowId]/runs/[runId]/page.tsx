"use client";
import { PhaseItemType, RunItemType } from "@vscrape/engine/src";
...
import { Separator, capitalizeFirstLetter, waitForElement } from "@vscrape/ui";
...
const WRunSinglePage = () => {
  ...
  const fakePhases: PhaseItemType[] = [];

  useEffect(() => {
    if (runId) {
      const runFound = ([] as RunItemType[]).find((r) => r.runId === runId);
